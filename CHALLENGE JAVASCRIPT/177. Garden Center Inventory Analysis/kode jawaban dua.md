...
function analyzeGardenInventory(plantRecords, supplyRecords) {
  ...
  // Analisis tanaman spesial
  for (const plant of plantRecords) {
    const nameLower = plant.name.toLowerCase();
    
    // Cek apakah nama tanaman mengandung 'helichrysum' atau 'acanthus'
    if (nameLower.includes('helichrysum') || nameLower.includes('acanthus')) {
      specialtyPlants++;
    }
  }
  ...
  // Analisis supply
  for (const supply of supplyRecords) {
    
    // Hitung total nilai (jumlah × harga)
    totalValue += supply.qty * supply.price;
    
    // Tandai item dengan stok rendah (≤ 5 unit)
    if (supply.qty <= 5) {
      lowStockItems++;
    }
    
    // Tandai item dengan harga tinggi (> $50)
    if (supply.price > 50) {
      highCostItems++;
    }
  }
